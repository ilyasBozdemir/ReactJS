...
export const WeatherProvider = ({ children }) => {
  ...
  const date = new Date();
   ...
   useEffect(() => {
      setCity(citiesJSON[69]);
      axios(
         `https://api.openweathermap.org/data/2.5/onecall?lat=37.1759&lon=33.2287&units=metric&exclude=current,minutely,hourly,alerts&lang=tr&appid=${API_KEY}`
      ).then((res) => {
         setData(res.data);
         setControl(true);
      });
